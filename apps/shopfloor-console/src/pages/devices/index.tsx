...
import { toKebabCase } from '../../util/util';
...
import { useDataStore, useFilterStore } from '../../store/device-store';
import { useDeviceStatus } from '../../store/hooks/device';
import AgGridTable from './components/ag-grid-table/ag-grid-table';
import DeviceDetails from './components/device-details/index';
...
function createUniqueValueArray(devices: Device[], key: string) {
  return Array.from(
    new Set(devices.map<string>((device) => device[key as keyof Device] ?? ''))
  );
}
